...
//防止一个线程创建多个EventLoop，thread_local保证每个线程只有一个副本
thread_local EventLoop *t_loopInThisThread = nullptr;

//定义默认Poller IO复用的超时时间 10s
const int kPollTimesMs= 10000;

//创建通知fd用来通知subReactor处理新来的Channel
int createEventfd()
{
    //创建事件通知fd，设置非阻塞和关闭执行模式
...
void EventLoop::loop()
{
    ...
    while (!quit_)
    {
        ...
        //监听两类fd一种是client的fd(客户单)，一种是wakeupfd(主循环和字循环通信fd，mainloop和subloop)
...
{
    quit_ = true;
    //如果是在其它线程中调用quit 在一个subloop(woker)中调用了mainLoop(IO)的quit
    if(!isInLoopThread())
    {
        wakeup();
    }
...
void EventLoop::runInLoop(Functor cb)
{
    ...
    else //在非当前loop中执行，唤醒loop所在线程，执行cb
    {
        queueInLoop(cb);
    }
}
...
void EventLoop::queueInLoop(Functor cb)
{
    ...
    /**
     * 唤醒相应的，需要执行上面回调函数操作的loop的线程
     * callingPendingFunctors_== ture 正在执行回调
     * 需要重新唤醒，因为有新的操作写入到pendingFunctors_中
     * 如果不重新唤醒则新写入的不会执行，
     * 下次写入时会执行上次写入的回调，显然不行
     */
